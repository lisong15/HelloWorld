...
(function (webim) {
        ...
        this.formatTimeStamp = function (time, format) {
            ...
            for (var k in o) {
                if (new RegExp("(" + k + ")").test(formatTime)) {
                    formatTime = formatTime.replace(RegExp.$1, (RegExp.$1.length == 1) ? (o[k]) :
    ...
    var getApiUrl = function (srvName, cmd, cbOk, cbErr) {
        ...
        var url = srvHost + '/' + SRV_NAME_VER[srvName] + '/' + srvName + '/' + cmd + '?websdkappid=' + SDK.APPID +
                  "&v=" + SDK.VERSION;
        ...
        url += '&sdkappid=' + ctx.sdkAppID + '&accounttype=' + ctx.accountType + '&apn=' + ctx.apn + '&reqtime=' +
               unixtime();
        return url;
    };
                            ...
                            function (resp) {
                                if (resp.JoinedStatus && resp.JoinedStatus == 'JoinedSuccess') {
                                    if (resp.LongPollingKey) {
                                        ...
                                        cbErr && cbErr(tool.getReturnError(
                                            "The type of group is not AVChatRoom: groupid=" + options.GroupId, -12));
                                        return;
                                    }
                                }
                                if (cbOk) {
                                    ...
                                    for (var i in errorAccount) {
                                        ...
                                        for (var j in resp.UserProfileItem) {
                                            if (resp.UserProfileItem[j].To_Account == failCount) {
                                                ...
                                                resp.ErrorInfo +=
                                                    "账号:" + failCount + "," + resp.UserProfileItem[j].ResultInfo + "\n";
                                                break;
                                            }
                                        }
                                    }
    ...
    Msg.Elem.Images.prototype.toHtml = function () {
        ...
        return "<img src='" + smallImage.getUrl() + "#" + bigImage.getUrl() + "#" + oriImage.getUrl() +
               "' style='CURSOR: hand' id='" + this.getImageId() + "' bigImgUrl='" + bigImage.getUrl() +
    ...
    Msg.Elem.File.prototype.toHtml = function () {
        ...
        return '<a href="' + this.downUrl + '" title="点击下载文件" ><i class="glyphicon glyphicon-file">&nbsp;' + this.name +
               '(' + fileSize + 'KB)</i></a>';
    };
    ...
    var MsgManager = new function () {
            ...
            for (var groupId in groupMsgMap) {
                ...
                if (curMaxMsgSeq) {//存在这个群的最大消息seq
                    ...
                    if (groupMsgMap[groupId].min - curMaxMsgSeq > 1 || groupMsgMap[groupId].msgs.length < tempCount) {
                        //发起一个拉群群消息请求
                        log.warn(
                            "发起一次补齐群消息请求,curMaxMsgSeq=" + curMaxMsgSeq + ", minMsgSeq=" + groupMsgMap[groupId].min +
                            ", maxMsgSeq=" + groupMsgMap[groupId].max + ", msgs.length=" +
                            groupMsgMap[groupId].msgs.length + ", tempCount=" + tempCount);
                    ...
                    if (groupMsgMap[groupId].msgs.length < tempCount) {
                        //发起一个拉群群消息请求
                        log.warn("发起一次补齐群消息请求,minMsgSeq=" + groupMsgMap[groupId].min + ", maxMsgSeq=" +
                                 groupMsgMap[groupId].max + ", msgs.length=" + groupMsgMap[groupId].msgs.length +
                                 ", tempCount=" + tempCount);
        ...
        this.bigGroupLongPolling = function (cbOk, cbErr) {
            ...
            proto_bigGroupLongPolling(opts, function (resp) {
                ...
                if (cbOk) {
                    cbOk(msgObjList);
                } else if (onBigGroupMsgCallback) {
                    onBigGroupMsgCallback(msgObjList);
                }//返回新消息
                    ...
                    for (var mi in msgInfo.MsgBody) {
                        ...
                        switch (msgType) {
                            ...
                            case MSG_ELEMENT_TYPE.FILE + " ":
                                ...
                                var fileUrl = getFileDownUrl(msgBody.MsgContent.UUID, msgInfo.From_Account,
        ...
        var handlerGroupMsg = function (msgInfo, isSyncGroupMsgs, isAddMsgFlag) {
            ...
            //群提示消息,重新封装下
            if (LONG_POLLINNG_EVENT_TYPE.GROUP_TIP == msgInfo.Event ||
        ...
        var getSoundDownUrl = function (uuid, senderId) {
            var soundUrl = null;
            if (authkey && ipList[0]) {
                soundUrl = "http://" + ipList[0] + "/asn.com/stddownload_common_file?authkey=" + authkey + "&bid=" +
                           DOWNLOAD_FILE.BUSSINESS_ID + "&subbid=" + ctx.sdkAppID + "&fileid=" + uuid + "&filetype=" +
                           DOWNLOAD_FILE_TYPE.SOUND + "&openid=" + senderId + "&ver=0";
            } else {
        ...
        var getFileDownUrl = function (uuid, senderId, fileName) {
            var fileUrl = null;
            if (authkey && ipList[0]) {
                fileUrl = "http://" + ipList[0] + "/asn.com/stddownload_common_file?authkey=" + authkey + "&bid=" +
                          DOWNLOAD_FILE.BUSSINESS_ID + "&subbid=" + ctx.sdkAppID + "&fileid=" + uuid + "&filetype=" +
                          DOWNLOAD_FILE_TYPE.FILE + "&openid=" + senderId + "&ver=0&filename=" +
    ...
    var FileUploder = new function () {
        ...
        this.submitUploadFileForm = function (options, cbOk, cbErr) {
            ...
            var uploadApiUrl = "https://pic.tim.qq.com/v4/openpic/" + cmdName + "?tinyid=" + ctx.tinyid + "&a2=" +
                               ctx.a2 + "&sdkappid=" + ctx.sdkAppID + "&accounttype=" + ctx.accountType +
                               "&contenttype=http";
    ...
    webim.login = webim.init = function (loginInfo, listeners, opts, cbOk, cbErr) {
        ...
        if (listeners.jsonpCallback) {
            jsonpCallback = listeners.jsonpCallback;
        }
        //登录
